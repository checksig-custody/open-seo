...
| **P18** | `vitest.local.config.ts` (new), `package.json` (4 scripts), `AGENTS.md`/`CLAUDE.md` (policy section)                                                                                     | Low-memory local test profile. Upstream's default sizes the fork pool from the CPU count, which on a workstation running this fork's other tooling exhausted RAM. Additive: the default profile and `test:ci` are unchanged, so CI behaviour is byte-identical. |
