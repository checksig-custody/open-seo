...
CREATE TABLE `si_budget_reservations` (
...
CREATE UNIQUE INDEX `si_budget_reservations_idempotency_idx` ON `si_budget_reservations` (`idempotency_key`);--> statement-breakpoint
-- The aggregate the authority reads on every authorization.
CREATE INDEX `si_budget_reservations_day_idx` ON `si_budget_reservations` (`budget_day`,`status`);--> statement-breakpoint
CREATE INDEX `si_budget_reservations_month_idx` ON `si_budget_reservations` (`budget_month`,`status`);--> statement-breakpoint
CREATE INDEX `si_budget_reservations_expiry_idx` ON `si_budget_reservations` (`status`,`expires_at`);
